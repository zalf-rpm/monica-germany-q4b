...
USER_MODE = "localConsumer"
#USER_MODE = "container"

PATHS = {
    # adjust the local path to your environment
    "localConsumer": {
        "local-path-to-data-dir": "C:/Users/stella/Documents/GitHub/monica-germany-q4b/monica-data/data/",
        "local-path-to-output-dir": "C:/Users/stella/Documents/GitHub/monica-germany-q4b/out/",
        "local-path-to-csv-output-dir": "C:/Users/stella/Documents/GitHub/monica-germany-q4b/csv-out/"
    },
    "container": {
...
def write_row_to_grids(row_col_data, row, ncols, header, path_to_output_dir, path_to_csv_output_dir, setup_id, row_col_2_lk):
            ...
            if not os.path.isfile(path_to_row_file):
                with open(path_to_row_file, "wb") as _:
                    ...
                    header = [
                        ...
                        "LAI-max","Yield-final","AbBiom-final",
                        "Emergence-doy","Emergence-year",
                        "Anthesis-doy","Anthesis-year",
                        ...
                        if rcd_val != -9999 and len(rcd_val) > 0:
                            ...
                            for cm_count, data in cell_data.iteritems():
                                row_ = [
                                    ...
                                    data["Yield-final"],
                                    data["AbBiom-final"],
                                    data.get("Emergence-doy", "NA"),
                                    data.get("Emergence-year", "NA"),
                                    data.get("Anthesis-doy", "NA"),
                                    data.get("Anthesis-year", "NA"),
                                    data.get("Maturity-doy", "NA"),
                                    data.get("Maturity-year", "NA"),
                                    data["Cycle-length"]#,
    ...
    output_grids = {
        ...
        "Yield-final": {"data" : make_dict_nparr(), "cast-to": "float", "digits": 1},
        "AbBiom-final": {"data" : make_dict_nparr(), "cast-to": "float", "digits": 1},
        "Emergence-doy": {"data" : make_dict_nparr(), "cast-to": "int"},
                    ...
                    for cell_data in rcd_val:
                        for cm_count, data in cell_data.iteritems():
                            for key, val in output_grids.iteritems():
                                ...
                                if key in data:
                                    cmc_and_year_to_vals[(cm_count, data["Harvest-year"])][key].append(data[key])
                                else:
                                    cmc_and_year_to_vals[(cm_count, data["Harvest-year"])][key] #just make sure at least an empty list is in there
...
def run_consumer(config, leave_after_finished_run = True):
    "collect data from workers"

    # select paths 
    paths = PATHS[config["mode"]]
    
    config["out"] = paths["local-path-to-output-dir"]
    config["csv-out"] = paths["local-path-to-csv-output-dir"]
    ...
    def process_message(msg):
        ...
        if msg["type"] == "finish":
            ...
            row_col_2_lk[(int(row), int(col))] = custom_id["lk_id"]
