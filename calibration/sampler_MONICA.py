...
def run_calibration(calib_id, clustering_file, run_cells_file, calibration_target, params_to_calibrate, crop, dumped_envs_folder, rep, cal_val_years_file, load_pheno_params):
    'run a single calibration id'
    print("starting calibration id" + str(calib_id))
    
    #calib_id = "7"

    '''
    clustering_file = "no_clustering_lks_SM.csv"#"no_clustering_lks_WW.csv"#
    run_cells_file = "sample_fullcover_SM.csv"#"sample_fullcover_WW.csv"#
    ...
    #load pheno params to be injected
    inj_pheno_pars = defaultdict(list)
    if load_pheno_params != "":
        pheno_params_path = script_path + '/opt_params/'+ load_pheno_params + "/"
        ph_params_files = [f for f in listdir(pheno_params_path) if isfile(join(pheno_params_path, f))]
        for f in ph_params_files:
            pheno_cl = int(f.split(".")[0].split("_")[-1]) #after dropping the clustering approach pheno_cl = yield_cl = de/lk. de=0
            with open(join(pheno_params_path, f)) as _:
                reader = csv.reader(_)
                for row in reader:
                    p={}
                    p["name"] = row[0]
                    p["array"] = row[1]
                    p["value"] = float(row[2])
                    inj_pheno_pars[pheno_cl].append(p)
    ...
    for clu_id, lk_clu in sim_map.iteritems():
        ...
        spot_setup = spotpy_setup_MONICA.spot_setup(params, sim_map[clu_id], clu_obs, calibration_target, dumped_envs_folder, inj_pheno_pars)
        #rep = 25
        ...
        opt_params_dir = script_path + '/opt_params/'+ calib_id + "/"
        obs_sim_dir = script_path + '/obs_vs_sim/'+ calib_id + "/"
        if not os.path.exists(opt_params_dir):
            os.makedirs(opt_params_dir)
        if not os.path.exists(obs_sim_dir):
            os.makedirs(obs_sim_dir)

        with open(opt_params_dir + crop + "_"  + calibration_target + '_optimizedparams_cl_' + str(clu_id) + '.csv', 'wb') as _:
            writer = csv.writer(_)        
            ...
            text='optimized parameters saved for calib id' + str(calib_id)
            print(text)
        
        with open(obs_sim_dir + crop + "_" + calibration_target + '_obs_sim_cl_' + str(clu_id) + '.csv', 'wb') as _:
            writer = csv.writer(_)
            ...
            for i in range(len(obs_list)):
                ...
                writer.writerow(outrow)
            text='obs_sim file saved for calib id' + str(calib_id)
            print(text)

    print("finished calibration id" + str(calib_id))

#run all calibrations from log file
with open(script_path + "/" + "calibration_log.csv") as _:
    ...
    for line in reader:
        ...
        load_pheno_params = line[9]

        run_calibration(calib_id, clustering_file, run_cells_file, calibration_target, params_to_calibrate, crop, dumped_envs_folder, rep, cal_val_years_file, load_pheno_params)
